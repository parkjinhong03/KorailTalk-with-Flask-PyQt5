...
def TrainWindow(self):
    ...
    specific_train_data = train_data['1']
    print(specific_train_data)
    layout_module.CreateTuple(self, specific_train_data['train_num'], specific_train_data['train_name'], specific_train_data['start_time'],
                              specific_train_data['end_time'], train_data['fare']['general'])


class layout_module:
    def CreateTuple(self, train_num, train_name, start_time, end_time, fare):
        print(train_num, train_name, start_time, end_time, fare)
...
class my_request:
    def Request_Train(self):
        ...
        date = my_request.Get_Date(self)  # -> SearchWindow 없이 바로 실행 시 현재 날짜가 반환됨
